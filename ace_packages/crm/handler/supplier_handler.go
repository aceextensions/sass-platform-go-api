...
func NewSupplierHandler(svc service.SupplierService) *SupplierHandler {
	return &SupplierHandler{service: svc}
}
...
func (h *SupplierHandler) List(c echo.Context) error {
	...
	suppliers, err := h.service.GetByTenantID(c.Request().Context(), tenantID, limit, offset)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
...
func (h *SupplierHandler) Search(c echo.Context) error {
	...
	suppliers, err := h.service.Search(c.Request().Context(), tenantID, query, limit, offset)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
