...
func (h *CustomerHandler) List(c echo.Context) error {
	...
	customers, err := h.service.GetByTenantID(c.Request().Context(), tenantID, limit, offset)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
...
func (h *CustomerHandler) Search(c echo.Context) error {
	...
	customers, err := h.service.Search(c.Request().Context(), tenantID, query, limit, offset)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
